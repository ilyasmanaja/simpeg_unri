            ...
            @foreach($antrian as $item)
              ...
              <div class="antrean-row">
                ...
                <div class="antrean-info">
                  ...
                  <div class="antrean-meta">NIP: {{ $nip }} <span class="chip-jabatan">{{ $perihal }}</span></div>
                </div>
              ...
              <div class="detail-overlay" id="overlay-st-{{ $item->id_verifikasi }}">
                <div class="detail-modal">
                  <div class="panel-head">
                    ...
                    <button class="btn-panel-close" onclick="tutupPanelST({{ $item->id_verifikasi }})"><i class="bi bi-x-lg"></i></button>
                  </div>
                  <div class="panel-body">
                    <div class="panel-section-title">Data Pengusul</div>
                    <div class="detail-grid">
                      <div class="detail-field"><span class="detail-label">Nama Pengusul</span><span class="detail-val">{{ $nama }}</span></div>
                    ...
                    <div class="detail-grid">
                      <div class="detail-field full"><span class="detail-label">Perihal</span><span class="detail-val highlight">{{ $st?->perihal ?? '-' }}</span></div>
                      <div class="detail-field full"><span class="detail-label">Tujuan</span><span class="detail-val">{{ $st?->tujuan ?? '-' }}</span></div>
                      <div class="detail-field"><span class="detail-label">Waktu Pelaksanaan</span><span class="detail-val">{{ $st?->waktu ?? '-' }}</span></div>
                      <div class="detail-field"><span class="detail-label">Nomor Surat</span><span class="detail-val">{{ $st?->nomor_surat ?? '-' }}</span></div>
                    </div>
                    @if($st && $st->anggota && $st->anggota->count())
                      ...
                      <div style="padding:4px 0 12px;display:flex;flex-wrap:wrap;gap:6px;">
                        @foreach($st->anggota as $anggota)
                          <span class="chip-jabatan">{{ $anggota->pegawai?->nama ?? '-' }}</span>
                        @endforeach
                      </div>
                    @endif
                    ...
                    <div id="tolak-st-{{ $item->id_verifikasi }}" style="display:none">
                      ...
                      <div class="tolak-block">
                        ...
                        <p class="catatan-hint" id="hint-st-{{ $item->id_verifikasi }}" style="display:none">
                  ...
                  <div class="panel-footer">
                    <form method="POST" action="{{ url('operator/verifikasi/' . $item->id_verifikasi . '/verifikasi') }}">
                      @csrf
                      <button type="submit" id="btn-verif-st-{{ $item->id_verifikasi }}" class="btn-verifikasi-modal">
                    ...
                    <form id="form-tolak-st-{{ $item->id_verifikasi }}" method="POST"
                      ...
                      @csrf
                      <input type="hidden" name="keterangan" id="input-catatan-st-{{ $item->id_verifikasi }}">
                      <button type="button" id="btn-tolak-st-{{ $item->id_verifikasi }}" class="btn-tolak-modal"
      ...
      <div class="tab-pane fade {{ request('tab') === 'riwayat' ? 'show active' : '' }}" id="panel-riwayat">
        ...
        <div class="table-responsive custom-table">
          <table class="table data-table mb-0">
            <thead>
              <tr>
                <th>No</th><th>Nama Pengusul</th><th>Perihal</th>
                <th>Status</th><th>Tgl Pengajuan</th><th>Tgl Diproses</th><th>Catatan</th>
              </tr>
            </thead>
            <tbody>
              @forelse($riwayat as $i => $item)
                ...
                <tr>
                  ...
                  <td>
                    <span class="badge-status {{ $item->status_verifikasi === 'Terverifikasi' ? 'badge-ok' : 'badge-tolak' }}">
              ...
              @empty
                <tr><td colspan="7" class="text-center py-5 text-muted">Belum ada riwayat</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
