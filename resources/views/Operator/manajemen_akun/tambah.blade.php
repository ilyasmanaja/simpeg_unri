...
@section('content')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // SYARAT PIMPINAN
            // =========================
            // Disesuaikan dengan value riil kolom `jenis_pangkat` di database (misal: "IV/a", "IV/b", dst)
            ...
            function updateFieldVisibility() {
                ...
                checkboxes.forEach(cb => {
                    if (
                        status === 'Non PNS' &&
                        (cb.dataset.jenis === 'dosen' || cb.dataset.jenis === 'pimpinan')
                    ) {
                        cb.checked = false;
                        cb.disabled = true;
            ...
            function updatePimpinanWarnings() {
                ...
                if (statusEl.value === 'PNS') {
                    ...
                    // Mengambil data-gol (contoh riil: "IV/a", "IV/b") lalu dibersihkan spasi-nya
                    const gol = selectedOpt ? (selectedOpt.dataset.gol || '').trim() : '';
                    
                    // Melakukan pencocokan substring atau string tepat sesuai data di DB Adminer
                    const pangkatOk = golonganPimpinan.some(g => gol.includes(g));
                    warnPangkat.style.display = pangkatOk ? 'none' : '';
                } else {
                    warnPangkat.style.display = 'none';
                }

                // CEK JABFUNG
                const selectedJabfungOpt = jabfungEl.options[jabfungEl.selectedIndex];
                const jabfungText = selectedJabfungOpt ? selectedJabfungOpt.text.toLowerCase().trim() : '';

                const jabfungOk = jabfungPimpinan.some(j => jabfungText.includes(j));
                warnJabfung.style.display = jabfungOk ? 'none' : '';
            }
            ...
            function updateRoleLogic() {
                ...
                if (statusEl.value === 'Non PNS') {
                    if (dosenCb) {
                        dosenCb.checked = false; dosenCb.disabled = true;
                        dosenCb.closest('.hak-akses-card').style.opacity = '.45';
                    }
                    if (pimpinanCb) {
                        pimpinanCb.checked = false; pimpinanCb.disabled = true;
                ...
                if (dosenCb && dosenCb.checked) {
                    if (tendikCb) {
                        tendikCb.checked = false; tendikCb.disabled = true;
                        tendikCb.closest('.hak-akses-card').style.opacity = '.45';
                    }
                    if (operatorCb) {
                        operatorCb.checked = false; operatorCb.disabled = true;
                        operatorCb.closest('.hak-akses-card').style.opacity = '.45';
                    }
                    if (pimpinanCb) {
                        pimpinanCb.disabled = false;
                        pimpinanCb.closest('.hak-akses-card').style.opacity = '1';
                    }
                }
                ...
                if (pimpinanCb && pimpinanCb.checked) {
                    ...
                    if (dosenCb) {
                        dosenCb.disabled = false;
                        dosenCb.closest('.hak-akses-card').style.opacity = '1';
                    }
                }
                ...
                if (operatorCb && operatorCb.checked) {
                    if (tendikCb) {
                        tendikCb.checked = true; tendikCb.disabled = true;
            ...
            // Buka disable sewaktu submit agar data checkbox yang ter-lock tetap terkirim ke backend
