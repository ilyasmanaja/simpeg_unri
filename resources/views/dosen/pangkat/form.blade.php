...
@php
    ...
    $formTitle = $isRevisi ? 'Revisi Pengajuan Pangkat dan Golongan'
               : ($isEdit  ? 'Edit Pengajuan Pangkat dan Golongan'
...
<div class="main">
    <div class="main-inner" style="padding-top: 32px;">
        <div class="container mb-5" style="max-width: 720px;">
            ...
            <div class="alert d-flex gap-2 align-items-start mb-3" style="border-left:4px solid #b91c1c;border-radius:10px;background:#fef2f2;">
            ...
            <div class="form-main-card">
                ...
                <div class="form-header-stripe">
                    <div class="fh-icon">
                        <i class="bi {{ $isRevisi ? 'bi-arrow-repeat' : ($isEdit ? 'bi-pencil-square' : 'bi-award-fill') }}"></i>
                    </div>
                    <div>
                        <h5>{{ $formTitle }}</h5>
                        <p>{{ $formSub }}</p>
                ...
                <div class="p-4">
                    @if ($isCreate && isset($sudahPuncak) && $sudahPuncak)
                        <div class="text-center py-4 text-muted">
                            ...
                            <a href="{{ route('dosen.pangkat-golongan.index') }}" class="btn btn-outline-secondary btn-sm">Kembali</a>
                        </div>
                    @else

                        {{-- Form Action --}}
                        @if ($isRevisi || $isEdit)
                            <form method="POST" action="{{ route('dosen.pangkat-golongan.update', $pengajuan->id_pengajuan) }}" enctype="multipart/form-data" id="formPanggol">
                            @method('PUT')
                            <input type="hidden" name="mode" value="{{ $mode }}">
                        @else
                            <form method="POST" action="{{ route('dosen.pangkat-golongan.store') }}" enctype="multipart/form-data" id="formPanggol">
                        @endif
                        @csrf
                        ...
                        <div class="mb-3 pb-3 border-bottom">
                            ...
                            <div class="row g-3 mb-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" value="{{ $identitas_value ?? '-' }}" readonly>
                                </div>
                            </div>
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <select name="target_panggol" id="selectPangkat" class="form-select" required>
                                    ...
                                    @foreach($semuaPangkat as $p)
                                        @php
                                            $isSelected = ($isEdit && $pengajuan->target_panggol === $p->id_panggol);
                                        @endphp
                                        ...
                                        @elseif($p->bisa || $isSelected)
                                            <option value="{{ $p->id_panggol }}" {{ $isSelected ? 'selected' : '' }}>
                                ...
                                <div class="rule-info mt-2">
                                    ...
                                    Kenaikan pangkat hanya dapat diajukan secara berurutan sesuai dengan batas maksimum Jabatan Fungsional Anda saat ini.
                                </div>
                            </div>
                        ...
                        <div class="mb-3 pb-3 border-bottom">
                            ...
                            <div class="mt-3">
                                ...
                                <input type="text" class="form-control" value="{{ $pengajuan->keterangan_tambahan }}" readonly>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="mb-4">
                            ...
                            <div class="info-box amber mb-3">
                                ...
                                <div>
                                    Hanya berkas yang <strong>ditandai bermasalah</strong> yang perlu diunggah ulang. Berkas lainnya tidak perlu diganti.
                                </div>
                            </div>
                            @elseif ($isEdit)
                            ...
                            <div class="berkas-section">
                                ...
                                <div class="berkas-item {{ $bermasalah ? 'bermasalah' : ($readonly ? 'readonly-item' : '') }}">
                                    <label for="file-{{ $bc['key'] }}">
                                        ...
                                        @if ($isRevisi)
                                            @if ($bermasalah)
                                                <span class="berkas-badge bermasalah-badge"><i class="bi bi-exclamation-circle me-1"></i>Perlu Diganti</span>
                                            @else
                                                <span class="berkas-badge ok-badge"><i class="bi bi-check-circle me-1"></i>Tidak Perlu Diganti</span>
                                            @endif
                                        @else
                                            <span class="berkas-badge {{ $bc['wajib'] ? '' : 'optional' }}">{{ $bc['wajib'] ? 'Wajib' : 'Opsional' }}</span>
                                        @endif
                                    </label>
                                    <div class="berkas-desc">{{ $bc['desc'] }}</div>

                                    @if ($fileAda)
                                        <a href="{{ Storage::url($fileAda['file_path']) }}" target="_blank" class="file-existing {{ $bermasalah ? 'file-existing-bermasalah' : '' }}">
                                    ...
                                    @if (!$readonly)
                                    <input type="file" name="{{ $bc['key'] }}" class="form-control mt-2" accept="application/pdf" id="file-{{ $bc['key'] }}">
                                    <small class="text-muted">PDF, maks. {{ $bc['max'] }}MB {{ $fileAda ? '— kosongkan jika tidak diganti' : '' }}</small>
                                    <div id="err-{{ $bc['key'] }}" class="text-danger mt-1" style="font-size:0.78rem; display:none;"></div>
                                    @else
                                    <div class="readonly-note"><i class="bi bi-lock me-1"></i> Berkas ini tidak perlu diganti</div>
                                    @endif
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if(typeof initFileValidation === 'function') {
                initFileValidation({{ ($isEdit || $isRevisi) ? 'true' : 'false' }}, {{ json_encode($berkasBermasalahArr) }});
            }
        });
    </script>
@endpush