...
@php
    ...
    $defaultJabfungId = ($isEdit || $isRevisi)
        ? $pengajuan->target_jabfung
        : ($jabfungTarget->id_jabfung ?? '');
@endphp
...
<div class="col main">
    <div class="main-inner" style="padding-top: 32px;">
        <div class="container mb-5" style="max-width: 720px;">
            ...
            {{-- Alert revisi: tampilkan alasan + berkas bermasalah --}}
            @if ($isRevisi)
            <div class="alert-revisi mb-3">
                ...
                @if (!empty($pengajuan->keterangan_tambahan))
                <div class="ar-alasan">
                    <span class="ar-label">Alasan:</span>
            ...
            <div class="form-main-card">
                ...
                <div class="p-4">
                    ...
                    <input type="hidden" name="id_jabfung" id="input-jabfung-id" value="{{ $defaultJabfungId }}">
                    ...
                    <div class="mb-3 pb-3 border-bottom">
                        ...
                        <div class="info-box {{ $jabfungNow ? 'green' : 'amber' }}">
                            <i class="bi bi-briefcase flex-shrink-0 mt-1"></i>
                            <div>
                                Jabatan fungsional saat ini:
                                <strong>{{ $jabfungNow ? $jabfungNow->nama_jabfung : 'Belum memiliki jabatan fungsional' }}</strong>
                                @if ($jenis === 'dosen' && isset($jabfungTarget))
                                    &nbsp;→&nbsp; Dapat mengajukan:
                                    <strong>{{ $jabfungTarget->nama_jabfung }}</strong>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 pb-3 border-bottom">
                        ...
                        @if ($jenis === 'dosen')
                        {{-- Dosen: dropdown dengan semua pilihan, non-target di-disable --}}
                        <div class="mb-3">
                            <label class="form-label-custom">
                                <i class="bi bi-briefcase me-1 text-danger"></i>Pilih Jabatan Fungsional
                            </label>
                            <select class="form-select" id="select-jabfung-dosen" disabled>
                                @foreach ($semuaJabfung as $jf)
                                    @php
                                        $u         = (int) $jf->urutan;
                                        $isTarget  = ($u === $urutanTarget);
                                        $isPast    = ($u < $urutanTarget);
                                        $isFuture  = ($u > $urutanTarget);
                                    @endphp
                                    <option value="{{ $jf->id_jabfung }}"
                                            {{ $isTarget ? 'selected' : '' }}
                                            {{ !$isTarget ? 'disabled' : '' }}>
                                        {{ $jf->nama_jabfung }}
                        ...
                        <div class="jabfung-ladder mb-3">
                            @foreach ($semuaJabfung as $jf)
                                @php
                                    $u = (int) $jf->urutan;
                                    if ($u < $urutanTarget)       $kelas = 'done';
                                    elseif ($u === $urutanTarget) $kelas = 'next';
                                    elseif ($u === $urutanNow)    $kelas = 'current';
                                    else                          $kelas = '';
                                @endphp
                                <div class="jl-step {{ $kelas }}">
                                    <span class="jl-label">{{ $jf->nama_jabfung }}</span>
                        ...
                        @else
                        {{-- Tendik: dropdown semua pilihan, jabfung saat ini di-disable --}}
                        <div class="mb-3">
                            <label class="form-label-custom">
                                <i class="bi bi-briefcase me-1 text-danger"></i>Pilih Jabatan Fungsional
                            </label>
                            <select class="form-select" id="select-jabfung-tendik"
                                ...
                                @foreach ($semuaJabfung as $jf)
                                    @php
                                        $isCurrent  = $jabfungNow && $jf->id_jabfung === $jabfungNow->id_jabfung;
                                        $isSelected = ($isEdit && isset($pengajuan) && $pengajuan->target_jabfung === $jf->id_jabfung);
                                    @endphp
                                    <option value="{{ $jf->id_jabfung }}"
                                            {{ $isCurrent ? 'disabled' : '' }}
                                            {{ $isSelected ? 'selected' : '' }}
                    ...
                    <div class="mb-3 pb-3 border-bottom">
                        ...
                        <div class="info-box blue">
                            <i class="bi bi-briefcase flex-shrink-0 mt-1"></i>
                            <div>
                                Jabfung yang direvisi: <strong>{{ $pengajuan->jabatanFungsional->nama_jabfung ?? '—' }}</strong>.
