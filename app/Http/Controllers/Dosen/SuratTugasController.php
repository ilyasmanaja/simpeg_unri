...
class SuratTugasController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('dosen.pengajuansurattugas.read_pengajuan_surat_tugas', compact('surat', 'pegawai'));
    }
    ...
    public function create()
    {
        ...
        return view('dosen.pengajuansurattugas.form_pengajuan_surat_tugas', [
            'pegawai' => $pegawai,
            'anggota' => [],
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        // 1. Simpan berkas ke disk dan database
        $berkas = $this->simpanBerkas($request, $surat->id_surat_tugas);

        // 2. Buat record antrean untuk dibaca oleh Operator
        if ($berkas) {
            Verifikasi::create([
                'id_berkas' => $berkas->id_berkas,
                'jenis_verifikasi' => Verifikasi::JENIS_SURAT_TUGAS,
        ...
        return redirect()->route('dosen.surat.index')
    ...
    public function prosesAjukanKembali(Request $request, $id)
    {
        ...
        // ==========================================
        // KONDISI 1: JIKA DITOLAK (VERIFIKASI)
        // ==========================================
        if ($statusLower === 'ditolak (verifikasi)') {
            ...
            $this->hapusBerkasSurat($id);
            $berkasBaru = $this->simpanBerkas($request, $id);

            // Update record Verifikasi agar masuk ke operator lagi
            if ($berkasBaru) {
                Verifikasi::updateOrCreate(
                    [
                        'id_berkas' => $berkasBaru->id_berkas,
                        'jenis_verifikasi' => Verifikasi::JENIS_SURAT_TUGAS
                    ],
                    [
                        'status_verifikasi' => 'Menunggu Diproses',
                        'tanggal_pengajuan' => Carbon::today(),
                        'keterangan' => 'Pengajuan hasil revisi (Verifikasi)',
                    ]
                );
            }
        ...
        // Update record Verifikasi untuk antrean Operator
        $berkasAktif = Berkas::where('id_surat_tugas', $id)->latest('id_berkas')->first();
        if ($berkasAktif) {
            Verifikasi::updateOrCreate(
                [
                    'id_berkas' => $berkasAktif->id_berkas,
                    'jenis_verifikasi' => Verifikasi::JENIS_SURAT_TUGAS
                ],
                [
                    'status_verifikasi' => 'Menunggu Diproses',
                    'tanggal_pengajuan' => Carbon::today(),
                    'keterangan' => 'Pengajuan hasil revisi',
                ]
            );
        }
    ...
    private function hapusBerkasSurat($idSurat): void
    {
        ...
        foreach ($berkasLama as $b) {
            ...
            // Hapus juga verifikasi terkait agar tidak ada data sampah di database
            Verifikasi::where('id_berkas', $b->id_berkas)->delete();
            $b->delete();
        }
    }
